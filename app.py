...
.stApp { background: #0b0b14; color: #e0e0ff; }
...
.risk-high { border-left-color: #ff4d6d !important; background: linear-gradient(135deg, #2a1a1e 0%, #1e1a2e 100%) !important; }
.risk-medium { border-left-color: #ffd166 !important; background: linear-gradient(135deg, #2a2a1a 0%, #1e1e2e 100%) !important; }
.risk-low { border-left-color: #06d6a0 !important; background: linear-gradient(135deg, #1a2a1e 0%, #1a1e2e 100%) !important; }
.priority-badge {
...
.stTabs [data-baseweb="tab-list"] { background: #1a1a2e; border-radius: 10px; padding: 4px; }
.stTabs [data-baseweb="tab"] { color: #9090cc; border-radius: 8px; }
.stTabs [aria-selected="true"] { background: #7c83fd22 !important; color: #c0c0ff !important; }
.stButton > button {
...
def load_data(uploaded=None):
    if uploaded is not None:
        ...
        try:
            ...
            st.error("bugs_data.csv not found. Please upload a file or run generate_data.py first.")
            st.stop()
...
def compute_risk_scores(df):
    ...
    grp["RiskLevel"] = pd.cut(grp["RiskScore"], bins=[-1,33,66,101],
...
def train_model(df):
    ...
    feature_cols = ["ModuleEnc","SeverityEnc","VersionEnc","Occurrences","Time to Fix (days)"]
    X = df2[feature_cols]
    y = df2["StatusEnc"]
    ...
    report     = classification_report(y_test, y_pred,
                    target_names=le_stat.classes_, output_dict=True)
    cm         = confusion_matrix(y_test, y_pred)
    importances = pd.Series(model.feature_importances_,
                    index=["Module","Severity","Version","Occurrences","Time to Fix"]
                  ).sort_values(ascending=True)
    encoders = {"module": le_mod, "severity": le_sev,
                "version": le_ver, "status": le_stat}
...
<div style='text-align:center; padding:20px 0 10px 0;'>
  ...
  <div style='font-size:1.1rem; font-weight:800; color:#c0c0ff; letter-spacing:1px;'>IATS</div>
  <div style='font-size:0.72rem; color:#6060aa; letter-spacing:2px; margin-top:2px;'>INTELLIGENT APP TESTING</div>
</div>
...
if page == "data_overview":
    ...
    # Missing values heatmap — pure Plotly
    st.markdown("#### Missing Values Heatmap")
    miss = df.isnull().astype(int)
    if miss.values.sum() == 0:
        st.success("✅ No missing values in the dataset.")
    else:
        fig_miss = px.imshow(miss, color_continuous_scale="Reds",
                             labels=dict(color="Missing"))
        plotly_layout(fig_miss, "Missing Values Heatmap", height=300)
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    fig4 = px.line(trend, x="App Version", y="Count", color="Severity", markers=True,
                   color_discrete_map={"High":"#ff4d6d","Medium":"#ffd166","Low":"#06d6a0"})
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    c1.metric("🔴 High Risk Modules",   (risk_df["RiskLevel"]=="High").sum())
    c2.metric("🟡 Medium Risk Modules", (risk_df["RiskLevel"]=="Medium").sum())
    c3.metric("🟢 Low Risk Modules",    (risk_df["RiskLevel"]=="Low").sum())

    st.markdown("#### Module Risk Score Table")
    display_df = risk_df[["Module","RiskScore","RiskLevel","TotalBugs",
                           "TotalOccurrences","ReopenRate","AvgFixTime"]].copy()
    display_df.columns = ["Module","Risk Score","Risk Level","Total Bugs",
                          "Total Occurrences","Reopen Rate","Avg Fix (days)"]
    ...
    def color_risk(val):
        c = {"High":"color:#ff4d6d;font-weight:bold",
    ...
    bar_colors = [color_map[str(lv)] for lv in risk_df["RiskLevel"]]
    fig_bar = go.Figure(go.Bar(
        x=risk_df["Module"], y=risk_df["RiskScore"],
        marker_color=bar_colors, text=risk_df["RiskScore"], textposition="outside",
        hovertemplate="<b>%{x}</b><br>Risk Score: %{y}<extra></extra>",
    ))
    plotly_layout(fig_bar, "Module Risk Scores", height=420)
    fig_bar.add_hline(y=66, line_dash="dash", line_color="#ff4d6d", annotation_text="High Threshold")
    fig_bar.add_hline(y=33, line_dash="dash", line_color="#ffd166", annotation_text="Medium Threshold")
    ...
    for _, row in top5.iterrows():
        vals = [
            float(norm_col("TotalOccurrences").loc[row.name]),
            float(norm_col("AvgSeverity").loc[row.name]),
            float(norm_col("ReopenRate").loc[row.name]),
            float(norm_col("AvgFixTime").loc[row.name]),
            float(norm_col("TotalBugs").loc[row.name]),
        ]
        vals += [vals[0]]
        radar_fig.add_trace(go.Scatterpolar(
            r=vals, theta=categories+[categories[0]],
            fill="toself", name=row["Module"], opacity=0.65,
        ))
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    st.markdown("Gradient Boosting classifier predicting **Bug Status** (Open / Fixed / Reopened).")
    st.markdown("---")
    ...
    tab1, tab2, tab3, tab4 = st.tabs(
        ["📈 Metrics","🔥 Confusion Matrix","📊 Feature Importance","🎯 Live Prediction"])
    ...
    with tab2:
        labels = list(encoders["status"].classes_)
        fig_cm = px.imshow(cm, x=labels, y=labels, text_auto=True,
                           color_continuous_scale="RdPu",
                           labels=dict(x="Predicted", y="Actual", color="Count"))
        plotly_layout(fig_cm, "Confusion Matrix", height=420)
        st.plotly_chart(fig_cm, use_container_width=True)

    with tab3:
        fig_imp = px.bar(importances.reset_index(),
    ...
    with tab4:
        ...
        if st.button("🔮 Predict Status"):
            try:
                ...
                icon  = {"Fixed":"✅","Open":"⚠️","Reopened":"🔁"}.get(pred_label,"🔮")
                st.markdown(f"""
                ...
                proba_df = pd.DataFrame({"Status": encoders["status"].classes_, "Probability": proba})
                fig_p = px.bar(proba_df, x="Status", y="Probability", color="Status",
                               color_discrete_map={"Fixed":"#06d6a0","Open":"#ffd166","Reopened":"#ff4d6d"})
    ...
    if top_terms is None:
        ...
        with col1:
            ...
            fig_kw = px.bar(
                ...
                x=top_terms.values[::-1], y=top_terms.index[::-1],
                orientation="h", color=top_terms.values[::-1],
        ...
        with col2:
            ...
            fig_cl = px.scatter(
                ...
                color_discrete_sequence=ACCENT_COLORS, opacity=0.8,
            )
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    reopen_threshold = df["Occurrences"].quantile(0.70)
    risky_bugs = df[
        (df["Status"].isin(["Reopened","Open"])) &
        (df["Occurrences"] >= reopen_threshold)
    ].sort_values("Occurrences", ascending=False).head(15)
    if risky_bugs.empty:
        st.info("No high-risk reappearing bugs found.")
    else:
        st.dataframe(
            risky_bugs[["Bug ID","Module","Bug Description","Severity","Status","Occurrences","App Version"]],
            use_container_width=True, hide_index=True)
    ...
    fig_ver = make_subplots(rows=1, cols=3,
                            subplot_titles=["Total Bugs","Avg Fix Time (days)","Quality Score"])
    fig_ver.add_trace(go.Bar(x=ver_quality["App Version"], y=ver_quality["TotalBugs"],
                             marker_color="#7c83fd", name="Total Bugs"), row=1, col=1)
    fig_ver.add_trace(go.Bar(x=ver_quality["App Version"], y=ver_quality["AvgFixTime"].round(1),
                             marker_color="#ffd166", name="Avg Fix Time"), row=1, col=2)
    fig_ver.add_trace(go.Bar(x=ver_quality["App Version"], y=ver_quality["QualityScore"],
                             marker_color="#06d6a0", name="Quality Score"), row=1, col=3)
    plotly_layout(fig_ver, "Version Quality Comparison", height=380)
    st.plotly_chart(fig_ver, use_container_width=True)

    st.dataframe(ver_quality.rename(columns={
    ...
    fastest_fix_ver    = ver_quality.sort_values("AvgFixTime").iloc[0]["App Version"]
    ...
    insights = [
        ...
        ("📦 Worst Version",  f"Version <b>{worst_version}</b> had the most bugs. Review changes from this release and add targeted regression coverage.", "risk-medium"),
        ("⚡ Fastest Fixes",  f"Version <b>{fastest_fix_ver}</b> achieved the fastest average fix time — study its sprint process and replicate it.", "risk-low"),
        ("🐛 Top Bug",        f"Bug <b>{highest_occ_bug['Bug ID']}</b> in <b>{highest_occ_bug['Module']}</b> occurred <b>{highest_occ_bug['Occurrences']}</b> times. Critical stability issue.", "risk-high"),
        ("📂 Open Backlog",   f"There are <b>{total_open}</b> open bugs currently unresolved. Clear these before shipping the next version.", "risk-medium"),
        ("⚠️ High Severity",  f"<b>{pct_high:.1f}%</b> of all bugs are High severity. Introduce stricter pre-release checklists and static code analysis.", "risk-medium"),
        ...
        ("🧪 Automation",     f"Modules with >20% reopen rates ({most_reopen_mod}) should have automated regression suites integrated into CI/CD.", "risk-medium"),
        ("📊 QA Priority",    f"Focus next sprint on {highest_risk_mod}, {most_reopen_mod}, and any module with >30 open occurrences.", "risk-high"),
    ]
    ...
    priority_df.insert(0, "Priority Rank", range(1, len(priority_df)+1))
    priority_df["Recommended Coverage"] = priority_df["RiskScore"].apply(
        lambda s: "≥90% + automated regression" if s >= 66
        else "≥75% + smoke tests" if s >= 33
        else "≥60% + exploratory"
    )
    priority_df["ReopenRate"] = priority_df["ReopenRate"].apply(lambda x: f"{x:.1%}")
    st.dataframe(priority_df.rename(columns={
